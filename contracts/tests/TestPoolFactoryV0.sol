...
contract TestPoolFactoryOld is
    IErrors,
    Initializable,
    UUPSUpgradeable,
    PausableUpgradeable,
    IStructs
{
        ...
        returns (address)
    {
        ...
        (uint48 discountValue, uint48 dicountColValue) = licenseDiscount(
            poolData._licenseId
        );
        ...
        ILendingPool(lendingPool).initialize(
            Data(
                ...
                (protocolFee * (hundredPercent - discountValue)) /
                    hundredPercent,
                (protocolColFee * (hundredPercent - dicountColValue)) /
                    hundredPercent,
                address(feesManager),
                oracle,
                address(this),
    ...
    function setImplementation(address _implementation) external whenNotPaused {
        onlyOwner();
    ...
    /// @notice Update treasury address
    function setFeesManager(address _feesManager) external whenNotPaused {
        onlyOwner();
        feesManager = IFeesManager(_feesManager);
    }

    /// @notice Update treasury address
    function setLicenseEngine(address _licenseEngine) external whenNotPaused {
        onlyOwner();
        licenseEngine = ILicenseEngine(_licenseEngine);
    }

    /// @notice Update VENDOR fee
    function setProtcolFee(uint48 _protocolFee) external whenNotPaused {
        onlyOwner();
        protocolFee = _protocolFee;
    }
    ...
    /// @return the discount amounts for a given license
    function licenseDiscount(uint256 _licenceId)
        private
        returns (uint48, uint48)
    {
        if (
            !licenseEngine.exists(_licenceId) ||
            msg.sender != licenseEngine.ownerOf(_licenceId)
        ) {
            return (0, 0);
        } else {
            (
                ...
                uint48 colDiscount,
                uint48 expiry
            ) = licenseEngine.licenses(_licenceId);
